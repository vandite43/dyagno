...
export async function POST(req: Request) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      // Recurring subscription (Starter / Pro)
    ...
    case "customer.subscription.updated": {
      ...
      const { data: existing } = await supabase.from("subscriptions").select("id").eq("stripe_subscription_id", sub.id).single();
      if (existing) {
...
function getPlanName(priceId: string): string {
  const starterIds = [process.env.STRIPE_STARTER_MONTHLY_PRICE_ID, process.env.STRIPE_STARTER_YEARLY_PRICE_ID];
